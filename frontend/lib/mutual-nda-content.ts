...
export type ContentSegment =
  ...
  | { type: "field"; field: keyof MutualNdaFormData; fallback: string };
...
const FALLBACKS: Partial<Record<keyof MutualNdaFormData, string>> = {
  purpose: "[Purpose]",
...
function parseSegments(template: string): ContentSegment[] {
  const segments: ContentSegment[] = [];
  let lastIndex = 0;
  let match: RegExpExecArray | null;

  FIELD_TOKEN.lastIndex = 0;
  while ((match = FIELD_TOKEN.exec(template))) {
    if (match.index > lastIndex) {
      segments.push({ type: "text", value: template.slice(lastIndex, match.index) });
    }
    if (match[1] !== undefined) {
      segments.push({ type: "text", value: match[1], bold: true });
    } else if (match[2] !== undefined) {
      const field = match[2] as keyof MutualNdaFormData;
      segments.push({ type: "field", field, fallback: FALLBACKS[field] ?? `[${field}]` });
    }
...
const RAW_CLAUSES: { title: string; body: string }[] = [
  {
    title: "Introduction",
...
export const MUTUAL_NDA_CLAUSES: Clause[] = RAW_CLAUSES.map((clause, index) => ({
  number: index + 1,
  title: clause.title,
  segments: parseSegments(clause.body),
}));

export const MUTUAL_NDA_ATTRIBUTION =
