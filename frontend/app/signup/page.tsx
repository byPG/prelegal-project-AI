...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ChatApiError ? err.message : "Couldn't sign up. Please try again.");
    } finally {
  ...
  return (
    <main className="mx-auto flex max-w-md flex-col gap-6 px-6 py-16 sm:px-10">
      <div>
        <h1 className="font-display text-2xl font-semibold text-brand-navy">Sign up</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <label className="flex flex-col gap-1 text-sm text-brand-navy">
          ...
          <input
            ...
            className="rounded-md border border-brand-navy/20 px-3 py-2 text-sm focus:border-brand-blue focus:outline-none"
          />
          <span className="text-xs text-brand-gray">At least 8 characters.</span>
        </label>
